...
proptest! {
    ...
    /// AC1.7: item reordering via deterministic swaps preserves every
    /// original TaskItemId across KDL round-trip.
    ///
    /// The test simulates `LoroMovableList.mov()` at the LoroValue level by
    /// permuting the items list with (from, to) swaps, then round-tripping
    /// through KDL. Every original id must appear exactly once in the output.
    ///
    ...
    fn reorder_preserves_item_ids(
        mut items in items_list_for_reorder(),
        swaps in swap_pairs_strategy(8),
    ) {
        // Collect original ids before permutation.
        ...
        // Apply deterministic swaps to the items list, clamping indices to
        // the actual list length to stay valid when the drawn n=8 upper bound
        // exceeds the actual list size.
        let n = items.len();
        ...
        // Build the reordered task-list LoroValue and round-trip through KDL.
        let task_list = make_task_list(items);
        let rt = kdl_round_trip(&task_list)
            .map_err(|e| TestCaseError::fail(e))?;
        ...
        prop_assert_eq!(
            rt_ids.len(),
            original_ids.len(),
            "item count changed across reorder+round-trip: original={:?} rt={:?}",
            original_ids,
            rt_ids,
        );
        ...
        prop_assert_eq!(
            sorted_original,
            sorted_rt,
            "item ids changed across reorder+round-trip: original={:?} rt={:?}",
            original_ids,
            rt_ids,
        );
    }
