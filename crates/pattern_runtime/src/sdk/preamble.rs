...
pub fn build_effect_stack_type(decls: &[EffectDecl]) -> String {
    ...
    concat!(
        ...
        "Diagnostics.Diagnostics, Port.Port]"
    )
    .to_string()
}
...
mod tests {
    ...
    fn preamble_contains_type_m_alias() {
        ...
        assert!(
            preamble.contains("File.File, Mcp.Mcp, Spawn, Diagnostics.Diagnostics, Port.Port]"),
            "missing File/Mcp/Spawn/Diagnostics/Port in type M"
        );
    ...
    fn build_effect_stack_type_produces_correct_string() {
        ...
        assert!(
            stack.ends_with("Diagnostics.Diagnostics, Port.Port]"),
            "expected Diagnostics.Diagnostics, Port.Port] at end; got: {stack}"
        );
    }
    ...
    fn build_for_preserves_canonical_row_order_in_type_m() {
        ...
        assert!(
            row.ends_with("Diagnostics.Diagnostics, Port.Port]"),
            "type M must end with Diagnostics.Diagnostics, Port.Port, got: {row}"
        );
    }
