...
impl AgentRegistry {
    ...
    pub fn register(
        ...
        self.entries.insert(id, AgentEntry { mailbox_tx: tx, status });
    }
    ...
    pub fn queue_for_draft(
        ...
        let entry = self.entries.get(id).ok_or_else(|| {
            RouterError::PersonaNotFound(id.clone())
        })?;
    ...
    pub fn drain_draft_queue(&self, id: &PersonaId) -> Vec<(Message, MessageOrigin)> {
        self.draft_queues
            .get(id)
            .map(|q| q.lock().expect("draft queue mutex poisoned").drain(..).collect())
            .unwrap_or_default()
    }
}
...
mod tests {
    ...
    fn make_tx() -> (mpsc::UnboundedSender<MailboxInput>, mpsc::UnboundedReceiver<MailboxInput>) {
    ...
    fn register_active_removes_stale_draft_queue() {
        ...
        assert_eq!(drained.len(), 0, "draft queue should be cleared on promotion");
    }
