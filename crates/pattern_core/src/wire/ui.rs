...
///
/// Client-minted using [`crate::types::ids::new_snowflake_id`].
...
pub struct AgentMessage {
    ...
    /// [`crate::types::turn::TurnInput`].
    pub parts: Vec<ContentPart>,
    /// Caller-supplied origin attribution. The daemon passes this through
    /// directly to [`crate::types::turn::TurnInput::origin`] — it does
...
pub enum WireMessageAttachment {
    ...
    /// `crate::types::port::PortId`) — not the raw event source's
...
/// Wire mirror of [`crate::fronting::FrontingSet`].
///
...
pub use crate::spawn::SpawnSource;
...
/// Snapshot of a [`FrontingSet`](crate::fronting::FrontingSet) for the wire.
///
...
pub struct SlashCommand {
    pub command: String,
    pub args: Vec<String>,
    /// Provenance of the invocation. `None` for legacy/unknown callers;
    /// `Some` for any caller that wants to attribute.
    ///
    /// The TUI sets `Author::Partner + Sphere::Private`. Plugins RELAYING a
    /// human's slash command set `Author::Partner` (if the discord user is the
    /// partner) or `Author::Human` (otherwise), with `Sphere` mapped from the
    /// channel kind (DM → Private, private guild channel → SemiPrivate, public
    /// → Public). Plugins acting AUTONOMOUSLY (not relaying a human) use
    /// `Author::Plugin { partner_authority }`.
    ///
    /// Trust decisions key off the (Author, Sphere) pair:
    /// - Partner + any Sphere → full trust (admin commands fine)
    /// - Human + Private → DM with a non-partner; trusted but not admin
    /// - Human + SemiPrivate → small group context; reduced trust
    /// - Human + Public → stranger; command set should be empty/read-only
    /// - Plugin + partner_authority=true → autonomous plugin with partner trust
    /// - Plugin + partner_authority=false → autonomous plugin, low trust
    ///
    /// The daemon never needs to know about discord (or any other plugin's)
    /// channel IDs to make these decisions.
    #[serde(default)]
    pub source: Option<crate::types::origin::MessageOrigin>,
}
...
mod tests {
    ...
    use crate::types::origin::{Partner, Sphere};
    ...
    fn agent_message_agent_origin_roundtrip() {
        use crate::types::origin::AgentAuthor;
        let msg = AgentMessage {
