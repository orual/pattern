//! V2 Memory System
//!
//! Memory value types, schema definitions, the `MemoryStore` trait, and the
//! concrete storage implementations: the LoroDoc-backed [`MemoryCache`] and
//! [`SharedBlockManager`]. Both take `Arc<pattern_db::ConstellationDb>`
//! directly (no auth-DB plumbing — that's a provider-side concern).
