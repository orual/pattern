...
- [x] **Env setup for Haskell eval worker** (follow-up from Phase 5 Task 20). **Resolved upstream**: tidepool-extract now bundles the prelude internally. No env var needed; the eval worker passes only the Pattern SDK dir (`sdk_dir`) to `EvalWorker::spawn_with_includes`. All dead prelude-dir references have been removed from pattern code and docs (Phase 6 Task C).
